...
Coronavirus disease 2019 (COVID-19) is a contagious desease caused by SARS-CoV-2. The main symptoms for COVID-19 are fever, cough, difficulty breathing, fatigue and diarrhea with an incubation period between 1 to 14 days. According to World Health Organization, there have been 452,201,564 confirmed cases including 6,029,852 deaths of COVID-19 since this pandemic started As of March 13 2022. Since around 1/3 of infected person are asymptomatic carrier, it would be really difficult for government and people to be prepared and free from infection. Therefore, reserachers are dedicated to find the factors which would be most significant to the amount of confirmed cases for each country. 
...
As a result, the significant factor level has been tested and selected. Also, further discussions are made for the models, data set, and the entire analysis process at the end of this project. 
...
Does the level of country development and different density level of population have a significant difference on the rate of new cases reported among the whole world? To be more specified, group the countries as developed countries(very high developing level), developing countries(high developing level and medium developing level) and underdeveloped countries(low developing level), to analyze is there any major influence of the level of development level on the amount of new cases. The second factor is to analyze the influence of different population density level on the amount of new cases. 
...
<br>The first data set is retrieved from World Health Organization, which contains the daily cases and deaths by date reported to WHO from 01/03/2020, the beginning of the pandemic, to 03/01/2022. There are `r nrow(covid)` rows and `r ncol(covid)` columns  in total, the part will be included in the project is: 
...
<br>The third data set has been used is retrieved from the International Organization for Standardization, which include the country name and country ISO code. It contains 249 rows and 3 columns. 
...
<br>The fourth data set is retrieved from Our World In Data, which include a complete list of human development index updated to the year of 2022 for each country/area. It contains 229 rows and 3 columns. 
...
According to the box plot above, it is clearly showing that there exist a difference for different level of both factor HDI and population density. And within each level of HDI, there exists difference for different population density. 
...
```
From the plot of pairwise confidence interval, we could conclude that we are 95% sure that all HDI level exist significant difference on confirmed cases amount; 
Besides, we are 95% confidence that for density level, the pair `'very sparse' & 'sparse'`, and `'very dense' & 'sparse'` exist significant difference on confirmed cases amount. 
...
To wrap up the result of the model in this analysis, it could be conclude that both factors Human Development Index and Population Density of an area/country would have an influence on the confirmed cases amount. 

***
## Discussion & Limitation

1. different country have different policy. 
2. people from poor area may not have enough attention on the pandemic. 
3. people's regular life cannot be garanteed, how could they consider the illness of this kind of uncontrolable disease. 
4. hard to quaranteen infected people, thus population density cannot reprensent each family. It may have cluster of infection within each family. 
...
The covid dataset: https://covid19.who.int/WHO-COVID-19-global-data.csv

The Population density dataset: https://ourworldindata.org/coronavirus

The iso code dataset: https://www.iso.org/obp/ui/#search

HDI dataset: https://ourworldindata.org/human-development-index

https://worldpopulationreview.com/country-rankings/hdi-by-country

https://en.populationdata.net/rankings/hdi/

https://statisticstimes.com/demographics/countries-by-population-density.php

https://www.usnews.com/news/best-countries/slideshows/countries-without-reported-covid-19-cases?slide=5
